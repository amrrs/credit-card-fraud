...
output$xg_fraud_box <- renderInfoBox({
  real_data <-
    predict_fraudxg() %>% group_by(predict) %>% tally(name = "count") %>% filter(predict == 1)
  infoBox(
    "Alerts",
...
output$xg_genuine_box <- renderInfoBox({
  real_data <-
    predict_fraudxg() %>% group_by(predict) %>% tally(name = "count") %>% filter(predict == 0)
  infoBox(
    "Genuine",
    paste0(real_data$count),
...
output$xg_datatable <- renderDataTable({
  data <-
    predict_fraudxg() %>% group_by(predict) %>% filter(predict == 1)
