...
ReadData <- function(file) { ## Function that read file
...
shinyServer(server <- function(input, output, session) {
  ...
  sampled_data <- reactiveFileReader(1000, session, "stream.csv", ReadData)
  
  predict_fraud <- reactive({
    data <- sampled_data() %>% select(-Time_hr)
    h2odata <- as.h2o(data)
    #h2o.predict(demo_model,h2oData) %>% as.data.table()
    prediction <- bind_cols(data,h2o.predict(model,h2odata) %>% as.data.frame())
    return(prediction)
  })
